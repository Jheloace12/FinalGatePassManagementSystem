...
const App: React.FC = () => {
  ...
  const handleLogin = (user: User) => {
    ...
    if (user.role === UserRole.SECURITY) setActiveTab('security');
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'dashboard': return <Dashboard passes={passes} user={currentUser} />;
      case 'request': return <RequestPass onSubmit={createPass} isLoading={isLoading} />;
      case 'my-passes': return <PassList passes={passes.filter(p => p.visitorId === currentUser.id)} onStatusChange={updatePassStatus} role={currentUser.role} />;
      case 'all-passes': return <PassList passes={passes} onStatusChange={updatePassStatus} role={currentUser.role} />;
      case 'security': return <SecurityPortal passes={passes} onUpdateStatus={updatePassStatus} />;
      case 'history': return <HistoryReports passes={passes} />;
      case 'visitors':
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {visitorsOnly.length === 0 ? (
                      <tr><td colSpan={3} className="py-12 text-center text-slate-400 italic">No visitors yet.</td></tr>
                    ) : (
      ...
      default: return <Dashboard passes={passes} user={currentUser} />;
    }
  };
